package com.toanthaycong.backend.features.authentication.filter;
    ...
    protected void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if (authorization == null || !authorization.startsWith("Bearer ")) {
                throw new ServletException("Không có token.");
            }

            String token = authorization.substring(7);

            if (jsonWebTokenService.isTokenExpired(token)) {
                throw new ServletException("Token đã hết hạn.");
            }
            ...
            response.getWriter().write("{\"message\": \"Token bị lỗi hoặc không có.\"}");
        }
    }
}